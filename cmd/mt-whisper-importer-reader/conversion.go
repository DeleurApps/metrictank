...
func (c *conversion) getPoints(retIdx int, spp, nop uint32) map[schema.Method][]whisper.Point {
	...
	for i := largestArchiveIdx; i >= smallestArchiveIdx; i-- {
		...
		if arch.SecondsPerPoint == spp {
			...
			for m, points := range incResolution(in, c.method, fakeAvg, arch.SecondsPerPoint, spp, rawRes) {
				for _, p := range points {
					adjustedPoints[m][p.Timestamp] = p.Value
				}
			}
		} else {
			for m, points := range decResolution(in, c.method, fakeAvg, arch.SecondsPerPoint, spp, rawRes) {
				for _, p := range points {
					adjustedPoints[m][p.Timestamp] = p.Value
				}
			}
...
func incResolution(points []whisper.Point, method schema.Method, fakeAvg bool, inRes, outRes, rawRes uint32) map[schema.Method][]whisper.Point {
...
func decResolution(points []whisper.Point, method schema.Method, fakeAvg bool, inRes, outRes, rawRes uint32) map[schema.Method][]whisper.Point {
	out := make(map[schema.Method][]whisper.Point)
